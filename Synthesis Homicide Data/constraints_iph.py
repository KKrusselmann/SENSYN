...
# custom constraint such that if typehom is partnerkilling, relat is partner of ex-partner
...
def is_valid(column_names, data, extra_parameter):
    ...
    # data is invalid if type is iph and relat is partner/expartner
...
# whenever relationship is partner/expartner and type is not iph, we will replace age with a likely category (most occuring)
...
def transform(column_names, data, extra_parameter):
    ...
    # get total props
    relat_props = data[col1][data[col2].isin(cat2)].value_counts(normalize=True)
    # drop child age props
    relat_props = relat_props.drop(cat1)
    # normalize so sums to 1
    relat_props /= relat_props.sum()
    # take samples
    sample = np.random.choice(
        relat_props.index, p=relat_props.values, size=(data[col2].isin(cat2)).sum()
    )
    # fill in sample
...
IPHConstraintClass = create_custom_constraint_class(
